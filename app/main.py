...
# ---- Novos helpers p/ recuperar caminho/URL da foto do membro ----------------


def _normalize_sources_from_photos_field(photos_value) -> List[tuple[str, str]]:
    """
    Converte o campo de fotos (string única ou lista) em fontes normalizadas:
      - ("storage", "membros/abc.jpg")  ou
      - ("url", "https://...")
    Regras:
      * URL pública do Supabase -> ("storage", relpath) se for do bucket esperado
    ...
    for s in values:
        ...
        if parsed and parsed[0] == settings.SUPABASE_STORAGE_BUCKET:
            _, rel = parsed
            out.append(("storage", _normalize_supabase_relpath(rel)))
            continue
        if s.lower().startswith("membros/"):
            out.append(("storage", s))
        elif s.lower().startswith("uploads/membros/"):
            out.append(("storage", s.split("uploads/", 1)[1]))
        else:
            out.append(("url", s))
    return out


def _first_storage_photo_path(photos_value) -> Optional[str]:
    """Retorna o primeiro caminho relativo no bucket (ex.: 'membros/xxx.jpg')."""
    for kind, value in _normalize_sources_from_photos_field(photos_value):
        if kind == "storage":
            return value
    return None


def _get_member_primary_photo(member_id: str) -> tuple[Optional[str], Optional[str]]:
    """
    Busca a primeira foto do membro no Supabase (campo configurado) e
    retorna (photo_path_relativo_no_bucket, photo_url_publica_se_disponivel).
    """
    try:
        sb = get_supabase()
        ...
        photos_val = rows[0].get(settings.MEMBERS_PHOTOS_COLUMN)
        rel = _first_storage_photo_path(photos_val)
        if not rel:
            return None, None

        # tenta gerar URL pública (se o bucket for público)
        try:
            pub = (
                get_supabase()
...
class IdentityCandidate(BaseModel):
    ...
    photo_path: Optional[str] = Field(
        None, description="Caminho relativo no bucket (ex.: membros/abc.jpg)"
    )
    photo_url: Optional[str] = Field(
        None, description="URL pública (se o bucket for público)"
    )
...
async def identity(req: IdentityRequest):
    ...
    try:
        ...
        for mid, dist in top:
            ...
            photo_path, photo_url = _get_member_primary_photo(mid)
            out.append(
                IdentityCandidate(
                    member_id=mid,
                    distance=dist,
...
async def verify(req: VerifyRequest):
    ...
    try:
        ...
        photo_path, photo_url = _get_member_primary_photo(req.member_id)
        return {
            "ok": True,
...
async def verify_file(
    ...
    try:
        ...
        if emb is None:
            raise HTTPException(status_code=400, detail="Nenhum rosto detectado.")
        dist = _distance(emb, ref.tolist())
        photo_path, photo_url = _get_member_primary_photo(member_id)
        return {
            "ok": True,
            "member_id": member_id,
