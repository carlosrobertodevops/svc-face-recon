...
from .indexer import (
    mem_index,
    build_index_from_members,
)  # usa o seu indexer.py com Redis e progress
from .repository import get_conn, upsert_member_embedding
from .supabase_client import get_supabase
from .utils import (
    load_image_from_bytes,
...
def _normalize_supabase_relpath(p: str) -> str:
    """Normaliza caminho do Storage para relpath do bucket (remove / e 'uploads/')."""
    p = (p or "").strip().lstrip("/")
    if not p:
        return p
    if p.lower().startswith("uploads/"):
...
def _to_google_drive_download(url: str) -> str:
    """Converte links 'view' do Google Drive em link de download direto."""
    if "drive.google.com" not in url:
        return url
    file_id = None
    m = _DRIVE_FILE_ID.search(url) or _DRIVE_ID_QUERY.search(url)
    if m:
        file_id = m.group(1)
    return (
...
async def _fetch_image_bytes(
    *,
    ...
    """
    Entrada padronizada de imagem.
    Prioridade:
      1) supabase_path (aceita relpath ou URL pública do Supabase; 'uploads/' opcional)
      2) image_url (Supabase público => via Storage; Google Drive 'view' => download)
      3) image_b64
    """
    # 1) SUPABASE_PATH
    if supabase_path:
...
def _primary_photo_path(member_id: str) -> Optional[str]:
    """
    Retorna um 'photo_path' preferencial do membro (relpath no bucket).
    Isso permite incluir 'photo_path' nos responses de identify/verify.
    """
    try:
        sb = get_supabase()
        sel = f"{settings.MEMBERS_PHOTOS_COLUMN}"
        resp = (
            sb.table(settings.MEMBERS_TABLE)
            .select(sel)
            .eq(settings.MEMBERS_ID_COLUMN, member_id)
        ...
        if not rows:
            return None
        photos = rows[0].get(settings.MEMBERS_PHOTOS_COLUMN)
        if not photos:
            return None
        paths = photos if isinstance(photos, list) else [photos]
        for p in paths:
            if not p:
                continue
            parsed = public_url_to_storage_path(p)
            if parsed and parsed[0] == settings.SUPABASE_STORAGE_BUCKET:
                _, rel = parsed
                return _normalize_supabase_relpath(rel)
            return _normalize_supabase_relpath(str(p))
    except Exception:
        return None
...
@app.get("/ops/status", response_model=OpsStatus, tags=["ops"])
async def ops_status():
    return OpsStatus(
        cache_embeddings=len(mem_index.embeddings),
        threshold=getattr(settings, "FACE_RECOGNITION_THRESHOLD", None),
        db_url_host_hint=(
...
async def index_all():
    """
    Reindexa a partir do Supabase: baixa fotos, gera embeddings, salva no Postgres e recarrega o cache.
    Compatível com o seu indexer.py (progress é objeto).
    """
    try:
        result_dict = await build_index_from_members()
        # Pydantic fará o parse do dict -> IndexResult -> IndexProgress
        return IndexResponse(ok=True, result=IndexResult(**result_dict))
    except Exception as e:
        return IndexResponse(ok=False, error=str(e))
...
async def identity(req: IdentityRequest):
    """
    Identifica a pessoa mais provável (ou top_k) comparando contra o cache.
    Retorna também 'photo_path' do membro.
    """
    thr = float(settings.FACE_RECOGNITION_THRESHOLD)
    try:
        b = await _fetch_image_bytes(
        ...
        cands: List[Tuple[str, float]] = []
        for mid, ref in mem_index.embeddings.items():
            d = _distance(emb, ref.tolist() if hasattr(ref, "tolist") else list(ref))
            cands.append((mid, d))
        ...
        for mid, dist in top:
            out.append(
                IdentityCandidate(
                    ...
                    matched=dist <= thr,
                    name=_fetch_member_name_from_supabase(mid),
                    photo_path=_primary_photo_path(mid),
                )
            )
...
async def verify(req: VerifyRequest):
    ...
    try:
        ...
        if emb is None:
            raise HTTPException(status_code=400, detail="Nenhum rosto detectado.")
        dist = _distance(emb, ref.tolist() if hasattr(ref, "tolist") else list(ref))
        return VerifyResponse(
            ok=True,
            member_id=req.member_id,
            name=_fetch_member_name_from_supabase(req.member_id),
            ...
            threshold=thr,
            matched=dist <= thr,
            photo_path=_primary_photo_path(req.member_id),
        )
    except FileNotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except HTTPException:
...
async def verify_file(
    ...
    try:
        ...
        return VerifyResponse(
            ...
            threshold=thr,
            matched=dist <= thr,
            photo_path=_primary_photo_path(member_id),
        )
    except HTTPException:
        raise
    except Exception as e:
